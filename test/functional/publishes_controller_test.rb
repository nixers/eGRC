...
class PublishesControllerTest < ActionController::TestCase
  ...
  test "should create publish" do
    assert_difference('Publish.count') do
      post :create, publish: { name: @publish.name, responsible: @publish.responsible, status: @publish.status }
    end
  ...
  test "should update publish" do
    put :update, id: @publish, publish: { name: @publish.name, responsible: @publish.responsible, status: @publish.status }
    assert_redirected_to publish_path(assigns(:publish))
  end
